chmod -R a-w connectors
chmod -R a-w core
chmod -R a-w manager
chmod -R a-w templates
chmod -R a+w manager/media/browser/mcpuk/tmp
find . -name '*.tpl' -exec chmod a-w {} \;
find . -name '*.inc' -exec chmod a-w {} \;
...
chmod a-w `pwd`
find ./assets/ -name '*.php' -exec chmod a-w {} \;
chmod a+w assets/cache/siteCache.idx.php
chmod a+w assets/cache/sitePublishing.idx.php
chmod a+w core/cache