...
class StarRating
{
     ...
     * @param int $allVotes Всего проголосовавших // KARATT
     *
     * @return array|bool|StarRatingResponse
     */
    public function setVote($vote, $id, $allVotes = 0)
    {
        $vote = (int) $vote;
        $id = (int) $id;
        ...
        if ($data) {
            $total  = $allVotes > 0 ? $allVotes * $vote : $data['total'] + $vote;
            $votes  = $allVotes > 0 ? $allVotes : $data['votes'] + 1;
            $rating = $total ? round($total / $votes, 2) : $vote;

            $this->db->update(compact('total', 'votes', 'rating'), $this->ratingTable, 'rid=' . $id);
        } else {
            $total = $allVotes > 0 ? $allVotes * $vote : $vote;
            $votes = $allVotes > 0 ? $allVotes : 1;
            $rating = $vote;
